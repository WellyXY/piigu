...
> 最後更新：2026-04-24（**position_w 前端改動終於生效** — 四層 bug 全修：server Pydantic 欄位 + actor `_ensure_base_loras` 呼叫 + `PersistentDiffusionStage.scalable_loras` kwarg + **server 改用 `position_weight=` alias 呼叫 actor 避開 per-position default fallback**；lift_clothes 升級到 v3 extended step 2750（MD5 d7410c7b...）+ 新 prompt；新增 cumshot position LoRA（11 個）；default 解析度 640×960；Gemma bf16 + eager attention；audio 層跳過；ENHANCE_BATCH_SIZE 32→8；ENHANCE_DEFLICKER 15→25）
...
Transformer（43GB，常駐 VRAM）
    ...
    └── 第二層：Position LoRA（熱切換，in-place add/subtract）
          ...
          ├── lift_clothes.safetensors     w=0.7  自訓練 v3 extended step2750（2026-04-24；0.7 實測最佳；nsfw/motion 前端預設 0.0/0.0）
...
| lift_clothes | lift_clothes.safetensors | **0.7** | **0.0** | **0.0** | 自訓練 v3 extended step2750（MD5 d7410c7b...，2026-04-24 升級，0.7 實測最佳）；DEFAULT_PROMPTS 改為訓練 caption `A female grips the hem... pink nipples and defined areolas... natural nipple texture clearly detailed...` + `--lift_clothes` trigger |
...
4. **Server 傳參名錯誤，per-position 預設覆寫前端：** Server 用 `position_w=lw.get("position", 0.8)` 呼叫 actor，但 actor `generate()` 有 `if position_weight is None: position_w = cfg.POSITION_LORA_WEIGHTS.get(position, position_w)` 的 fallback。因為 `position_weight` 沒被傳（kwarg 名不同），永遠是 None → fallback 觸發 → **前端傳的 position_w 被 config default 靜默覆蓋**（e.g. lift_clothes 一律被拉回 0.6）。Worker log `pos_w=0.5` 但 server `swap_position_lora: already at ('lift_clothes', 0.6), skip`。
   修：Server 改用 alias `position_weight=/nsfw_weight=/motion_weight=` 呼叫 (`commit a200116`)

**驗證（curl 實測）：** 前端 nsfw=0/motion=0/pos_w=0.5 → log 出現 `[PIPELINE] swap_base_loras: rescaling 'nsfw' 1.000 -> 0.000 (magnitude=6.9M)` + `swap_position_lora: ('cowgirl', 0.8) -> ('lift_clothes', 0.5)`。反向切回 1.0/0.7 也 OK；連續同樣 weight 是 no-op。
